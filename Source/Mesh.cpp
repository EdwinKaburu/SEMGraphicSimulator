...
namespace {
    ...
    uniform float isRotate = 1;
    
    // Rotation About the Y-Axis
    vec3 Rotate3D(vec3 v)
    {
        // Define cos and sin theta
        float c0 = cos(radAng), s0 = sin(radAng);
        // Matrix - Vector Multiplication
        return vec3( (c0 * v.x + s0 * v.z),
                     v.y,
                     (-s0 * v.x + c0 * v.z));
    }
    void main() {

        // Apply Rotation of Points
        vec3 p = point;
        vec3 n = normal;
        vec3 uas = uaxis;
        vec3 uvs = vaxis;

        // Do we Rotate About Y - Axis ?
        if(isRotate == 1)
        {
            p = Rotate3D(point);   
            n = Rotate3D(normal);
        ...
        vPoint = (modelview*vec4(p, 1)).xyz;
        vNormal = (modelview*vec4(n, 0)).xyz;
        vUaxis = (modelview*vec4(uas, 0)).xyz;
        vVaxis = (modelview * vec4(uvs, 0)).xyz;   
    ...
    float PhongIntensity(vec3 pos, vec3 N) {
        
        float a = .15f, d = 0, s = 0;

        vec3 E = normalize(pos);            // eye vector

        vec3 L =  normalize(localLights == 1? light - pos : light);    // Light Vector

        vec3 R = reflect(L, N);             // highlight vector
       
        // d = abs(dot(N, L));              // two-sided diffuse

        d =  max(0, dot(N, L));             // one-side diffuse

        float h = max(0, dot(R,E));          //abs(dot(R, E));           // highlight term
   ...
   void main() {
        ...
        vec3 nn = TransformToLocal(n, vUaxis, vVaxis, vNormal);
        float intensity = PhongIntensity(vPoint, nn);
        vec3 color = useTexture == 1? texture(textureMap, vUv).rgb : vec3(1);
...
void Mesh::Buffer() {
    ...
    int sizePts = points.size() * sizeof(vec3),
        sizeNorms = normals.size() * sizeof(vec3),
        sizeTex = uvs.size() * sizeof(vec2),
        sizeUs = us.size() * sizeof(vec3),
        sizeVs = vs.size() * sizeof(vec3);
        

    glBufferData(GL_ARRAY_BUFFER,(sizePts + sizeNorms + sizeTex + sizeUs + sizeVs), NULL, GL_STATIC_DRAW);
    ...
    glBufferSubData(GL_ARRAY_BUFFER, sizePts + sizeNorms, sizeTex, &uvs[0]);

    glBufferSubData(GL_ARRAY_BUFFER,  sizePts + sizeNorms + sizeTex, sizeUs, &us[0]);
    glBufferSubData(GL_ARRAY_BUFFER, sizePts + sizeNorms + sizeTex + sizeUs, sizeVs, &vs[0]);

}

void Mesh::Display(CameraAB& camera, bool rotate) {
   ...
   // int sizePoints = nPts * sizeof(vec3);
    ...
    //int sizeUvs = nUvs * sizeof(vec2);

    //int sizeTex = textures.size() * sizeof(vec2);
    //int sizeTex = textures.size() * sizeof(vec2);
    ...
    if (rotate)
    {
       SetUniform(shader, "isRotate", (1.0f));
    }
    else
    {
         SetUniform(shader, "isRotate", (0.0f));
    }
    ...
    VertexAttribPointer(shader, "uaxis", 3, 0, (void*)(sizePts + sizeNorms + sizeTex));
    VertexAttribPointer(shader, "vaxis", 3, 0, (void*)(sizePts + sizeNorms + sizeTex + sizeUs));
    ...
    if (textureUnit) {
        ...
        SetUniform(shader, "textureMap", (int)textureName);
    }

    if (normalUnit)
    {
        glActiveTexture(GL_TEXTURE1 + normalUnit);  // active texture corresponds with textureUnit or textureName????
        glBindTexture(GL_TEXTURE_2D, normalName); // bound texture and shader id correspond with textureName
        SetUniform(shader, "bumpMap", (int)normalName);
    }
